...
**Mechanism:** the n8n container mounts this host directory at `/home/node/outputs/` (see `docker-compose.yml`). Mounted under `/home/node/` (the container user's home directory) so the n8n process — running as uid 1000 — has write permission. Earlier attempts at `/files/outputs` failed (parent is read-only) and `/outputs` failed (Windows bind mount didn't grant write to the non-root container user); the home-directory path is the reliable target. The workflow's `Build Markdown Memo` node renders the structured memo + evaluator + RFD + portfolio fit into a single Markdown document, and the `Write Memo File` node writes it via the n8n Read/Write File node.
